...
static ssize_t ap3216c_read(struct file *filp, char __user *buf, size_t cnt, loff_t *off)
{
	...
	struct cdev *cdev = filp->f_path.dentry->d_inode->i_cdev;
	struct ap3216c_dev *dev = container_of(cdev, struct ap3216c_dev, cdev);	//获取struct ap3216c_dev的首地址
...
static const struct file_operations ap3216c_ops = {
 ...
 /*
  * @description     : i2c驱动的probe函数，当驱动与设备匹配以后此函数就会执行;初始化设备与寄存器
  * @param - client  : i2c设备
  * @param - id      : i2c设备ID
  * @return          : 0，成功;其他负值,失败
  */
static int ap3216c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* set/get()方法：保存ap3216cdev结构体：将 ap3216cdev 变量的地址绑定到 client； */
...
{
	struct ap3216c_dev *ap3216cdev = i2c_get_clientdata(client);	//得到 ap3216cdev 变量的地址
	/* 注销字符设备驱动 */
	/* 1、删除cdev */
	cdev_del(&ap3216cdev->cdev);
	/* 2、注销设备号 */
...
/* 传统匹配方式ID列表,无设备树,只写属性值，不写实现方法，实际实现由I2C核心完成*/
static const struct i2c_device_id ap3216c_id[] = {
	{"alientek,ap3216c", 0},  
	{}
};

/* 设备树匹配列表,,只写属性值，不写实现方法，实际实现由I2C核心完成 */
static const struct of_device_id ap3216c_of_match[] = {
	{ .compatible = "alientek,ap3216c" },
	{ /* Sentinel */ }
};

/* i2c驱动结构体：首先进行match(有无设备树两种),match成功之后probe;卸载时执行remove */	
...
/*
 * @description	: 驱动入口函数，调用add向内核注册驱动
 */
