...
版本	   	: V1.0 2024 0114
描述	   	: gpio子系统驱动LED灯。
其他	   	: 使用信号量来实现对实现设备的互斥访问，就是使用semaphore实现一次只能允许一个应用访问LED;
			：
***************************************************************/
...
static int led_open(struct inode *inode, struct file *filp)
{
	...
	{ /* 获取信号量,这时count就为0;进入休眠状态的进程可以被信号打断 */
		return -ERESTARTSYS;
	}
#if 0
	down(&gpioled.sem);		/* down（）函数的解释，不能被中断；所以上面选用了down_interruptible */
#endif
