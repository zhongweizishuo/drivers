...
#define LEDOFF 0			   /*关灯*/
#define LEDON 1
#define LEDTWINKE 3 			/*灯闪烁*/

/* 2. 定义硬件设备的结构体 （gpioled）*/
struct gpioled_dev
{
	dev_t devid;
	int major, minor;
...
static int __init led_init(void)
{
	...
	/* 1、创建设备号 */
	if(gpioled.major){
		gpioled.devid = gpioled.MKDEV(gpioled.major,0);
		...
		ret = alloc_chardev_region(&gpioled.devid, 0, GPIOLED_CNT, GPIOLED_NAME);
		if(ret < 0){
